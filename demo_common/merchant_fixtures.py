...
def metric_window(rows: DailyRows, period: str | None) -> tuple[DailyRows, DailyRows, str]:
    """The rows for a period (7 days unless it names a number of days or an ISO date range)
    plus the prior window of the same length and the period's label."""
    days = 7
    if period:
        cleaned = period.strip().lower()
        span = re.fullmatch(r"(?:last[_ ])?(\d{1,3})(?:d|[_ ]days)", cleaned)
        if span:
            days = max(1, min(int(span.group(1)), len(rows)))
        elif cleaned in {"quarter"}:
