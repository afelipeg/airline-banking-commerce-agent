...
/** Renders present_order_status as the status of a booking. */
...
const STATUS_STYLE: Record<string, { className: string; pulse?: boolean }> = {
  ...
  out_for_delivery: { className: "am-tag--accent", pulse: true },
  processing: { className: "", pulse: true },
  delayed: { className: "am-tag--warn", pulse: true },
...
export default function BookingStatusCard({ payload }: { payload: OrderStatusPayload }) {
  ...
  return (
    <Frame component="order_status" label={`Reserva ${payload.order_id}`}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {order?.estimated_delivery ? (
          <p className="am-mono text-[13px] text-(--ink-soft)">
            Viaje <span className="font-semibold text-(--ink)">{formatDate(order.estimated_delivery)}</span>
          </p>
        ) : null}
      </div>
      ...
      {order?.items?.length ? (
        <div className="am-rule mt-3 pt-2">
          {order.items.map((item) => (
            <div key={item.product_id} className="flex items-baseline justify-between gap-3 py-1 text-[14px]">
              <span className="min-w-0 truncate text-(--ink)">
                {item.title}
                {item.quantity > 1 ? <span className="am-mono text-(--ink-soft)"> × {item.quantity}</span> : null}
              </span>
              <span className="am-mono shrink-0 text-(--ink)">{formatPrice(item.price * item.quantity)}</span>
            </div>
          ))}
          <div className="am-rule-bold mt-1 flex items-baseline justify-between pt-1.5">
            <span className="am-meta">Total</span>
            <span className="am-mono text-[15px] font-semibold text-(--ink)">{formatPrice(order.total, order.currency)}</span>
          </div>
        </div>
      ) : null}

      {payload.next_step ? (
        <p className="mt-3 border-l-2 border-(--accent) pl-3 text-[14px] font-medium text-(--ink)">{payload.next_step}</p>
      ) : null}
    </Frame>
  );
}
