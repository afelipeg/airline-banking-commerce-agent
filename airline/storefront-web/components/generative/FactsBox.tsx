...
/** The total row get_disclosure authors, in either language; it renders heavier. */
const ALL_IN_LABELS = new Set(["Estimated all-in", "Costo total estimado", "Total estimado", "Costo total"]);
...
export default function FactsBox({ payload }: { payload: DisclosurePayload }) {
  ...
  return (
    <Frame component="disclosure" label="Datos clave">
      <div className="mx-auto max-w-md border-x-2 border-(--ink) px-4 pb-3">
        ...
        <div className="am-rule-bold mt-2">
          {payload.rows.map((row, index) => {
            ...
            const isAllIn = ALL_IN_LABELS.has(row.label);
            return (
              <div
                key={`${row.label}-${index}`}
        ...
        {payload.sources?.length ? (
          <div className="am-rule mt-2 flex flex-wrap items-center gap-1.5 pt-2">
            <span className="am-meta">Fuentes</span>
