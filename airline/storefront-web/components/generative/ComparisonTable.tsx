...
/** Renders present_comparison; cabins, card tiers and insurance plans use plan_matrix. */
...
const RECOMMENDED_LABEL = "Recomendado";
...
export default function ComparisonTable({
  ...
  return (
    <Frame
      ...
      label={payload.title ?? "Lado a lado"}
      flush
    >
              ...
              <div className="self-end">
                {best_for ? (
                  <p className="am-rule pt-2 text-[13px] font-medium text-(--ink-soft)">
                    Ideal para: <span className="text-(--ink)">{best_for}</span>
                  </p>
                ) : null}
              </div>
            </article>
        ...
        <div className="border-t border-(--line) px-4 py-2.5">
          ...
          {payload.dimensions?.length ? (
            <p className="mt-0.5 text-[12px] text-(--ink-soft)">
              Comparado por: {payload.dimensions.join(" · ")}
            </p>
          ) : null}
        </div>
      ) : null}
