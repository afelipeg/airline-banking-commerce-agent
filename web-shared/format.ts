...
export function formatWeekday(value: string | null | undefined): string {
  ...
  return date.toLocaleDateString("es-CO", { weekday: "short", month: "short", day: "numeric", year });
}
...
export function coverLabel(days: number): string {
  return days < 1 ? "<1 día de cobertura" : `${plural(Math.round(days), "día")} de cobertura`;
}

/** "sells out in ~4 days", "sells out within a day". */
export function runwayLabel(days: number): string {
  return days < 1 ? "se agota en menos de un día" : `se agota en ~${plural(Math.round(days), "día")}`;
}
...
export function priceLabel(product: OptionFields): string {
  ...
  return hasOptions(product) ? `Desde ${money}` : money;
}
...
export function formatComparisonLabel(
  ...
  if (primary && compare) {
    ...
    if (primaryDays === compareDays && Math.round((primaryStart - compareEnd) / dayMs) === 1) {
      return primaryDays === 6 ? "semana anterior" : "periodo anterior";
    }
  }
...
export function describeProposer(change: {
  ...
  return change.created_by_kind === "agent"
    ? `Propuesto por el asistente de ${change.created_by}`
    : `Preparado por ${change.created_by}`;
}
...
export function describeResolver(change: {
  ...
  if (change.status === "applied" && change.applied_by) return `Aprobado por ${change.applied_by}`;
  if (change.status === "discarded" && change.discarded_by) {
    return change.discarded_by_kind === "agent"
      ? `Descartado por el asistente de ${change.discarded_by}`
      : `Descartado por ${change.discarded_by}`;
  }
  return null;
}
