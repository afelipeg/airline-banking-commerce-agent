...
const GATE_LABELS: Record<string, string> = {
  provenance: "control de procedencia",
  approval: "control de aprobación",
  guardrail: "límite de seguridad",
};
...
function trailing(row: ToolRow, status: RowStatus): string {
  switch (status) {
    case "running":
      return "en curso…";
    case "blocked":
      return `retenido · ${GATE_LABELS[row.reason ?? ""] ?? "control de seguridad"}`;
    case "error":
      return "error";
    default:
    ...
    <li>
      ...
      {open ? (
        <div className="mb-2 ml-5 space-y-2">
          {row.input ? <Detail label="Entrada" tone="bg-(--well)/70 text-(--ink)" text={row.input} /> : null}
          {row.excerpt !== undefined ? (
            <Detail label="Resultado (extracto)" tone={RESULT_TONE[status]} text={row.excerpt || "(vacío)"} />
          ) : row.result !== undefined ? (
            <Detail label="Resultado" tone={RESULT_TONE[status]} text={row.result || "(vacío)"} />
          ) : null}
        </div>
      ) : null}
    </li>
      ...
      <aside className="fixed inset-y-0 right-0 z-50 flex w-[min(94vw,400px)] flex-col border-l border-(--line) bg-(--card) shadow-2xl">
        <div className="flex items-start justify-between gap-3 border-b border-(--line) px-4 py-3">
          <div className="flex min-w-0 items-start gap-2">
            {turnCount > 1 ? (
              <span className="flex shrink-0 items-center gap-0.5" role="group" aria-label="Respuesta">
                <button type="button" onClick={() => stepTo(turn - 1)} disabled={turn <= 1} aria-label="Respuesta anterior" className={stepButton}>
                  ‹
                </button>
                <button type="button" onClick={() => stepTo(turn + 1)} disabled={turn >= turnCount} aria-label="Respuesta siguiente" className={stepButton}>
                  ›
                </button>
              </span>
            ) : null}
                ...
                {turnCount === 0 ? (
                  ...
                  <>
                    <span className="font-bold">
                      Respuesta {turn}
                      {turnCount > 1 ? <span className="font-normal text-(--ink-soft)"> de {turnCount}</span> : null}
                    </span>
                    <span className="font-normal text-(--ink-soft)">
                      {working ? (
                        ...
                        <>
                          · {rows.length} paso{rows.length === 1 ? "" : "s"}
        ...
        <div className="panel-scroll flex-1 overflow-y-auto px-4 py-3">
          <section>
            ...
            {turnCount === 0 ? (
              <Empty>Aún no hay respuestas.</Empty>
            ) : rows.length === 0 ? (
              <Empty>{working ? "Pensando…" : "Esta respuesta no usó herramientas."}</Empty>
            ) : (
          ...
          <section className="mt-5 border-t border-(--line) pt-4">
            <Heading>
              {memoryTitle}
              {newCount ? <span className="font-normal text-(--ink-soft)"> · {newCount} nuevos en esta sesión</span> : null}
            </Heading>
            {memory.length === 0 ? (
              <Empty>Nada guardado todavía.</Empty>
            ) : (
              <ul className="mt-1 space-y-1">
                {memory.map((fact) => (
                  <li key={fact.key} className="text-[13px] leading-snug text-(--ink)">
                    {fact.value}
                    {newMemoryKeys.has(fact.key) ? <em className="ml-1.5 text-(--ink-soft)">nuevo</em> : null}
                  </li>
                ))}
              </ul>
            )}
