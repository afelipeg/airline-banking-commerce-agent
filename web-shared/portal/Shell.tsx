  ...
  return (
    <div className="flex h-dvh bg-(--ground) text-(--ink)">
      <aside className="hidden w-16 shrink-0 flex-col border-r border-(--line) bg-(--chrome) px-2 py-3.5 lg:flex xl:w-[220px] xl:px-3">
        ...
        <nav className="flex flex-col gap-0.5" aria-label="Vistas del portal">
          {nav.map((item) => {
        ...
        <button
          ...
          aria-label={assistantOpen ? "Ocultar asistente" : "Mostrar asistente"}
          title={assistantOpen ? "Ocultar asistente" : "Mostrar asistente"}
        ...
        >
          ...
          <span className="hidden flex-1 xl:block">Asistente</span>
          {assistantBusy ? (
            <span className="relative hidden h-2 w-2 xl:flex" aria-label="Pensando">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex items-center gap-3 border-b border-(--line) bg-(--chrome) px-3 py-2 lg:hidden">
          {brand.mark}
          <nav className="panel-scroll flex min-w-0 flex-1 gap-1 overflow-x-auto" aria-label="Vistas del portal">
            {nav.map((item) => (
              <button
                key={item.id}
                type="button"
