...
export const CHANGE_STATUS: Record<ChangeLike["status"], { tone: Tone; label: string }> = {
  staged: { tone: "violet", label: "Por aprobar" },
  applied: { tone: "ok", label: "Aprobado" },
...
export function useChangeActions<T extends ChangeLike>(
  ...
  const act = async (action: ChangeAction) => {
    ...
    else setError("La acción no se completó. Revisa la API e inténtalo de nuevo.");
    setBusy(null);
  };
      ...
      {change.status === "staged" ? (
        <div className="flex flex-wrap items-center gap-2">
          <Button variant="accent" size="sm" icon="check" onClick={() => onAct("apply")} disabled={busy !== null || !canAct}>
            {busy === "apply" ? "Aplicando…" : "Aprobar"}
          </Button>
          <Button variant="secondary" size="sm" onClick={() => onAct("discard")} disabled={busy !== null || !canAct}>
            {busy === "discard" ? "Descartando…" : "Descartar"}
          </Button>
          <span className="text-[11.5px] leading-tight text-(--ink-soft)">Nada se aplica hasta que lo apruebes.</span>
        </div>
      ) : (
        <div className="flex items-center gap-2 text-[13px] text-(--ink-soft)">
          <Icon name={change.status === "applied" ? "check" : "x"} size={15} className={change.status === "applied" ? "text-(--ok)" : "text-(--ink-faint)"} />
          {change.status === "applied"
            ? `Aprobado${change.applied_by ? ` por ${change.applied_by}` : ""}${change.applied_at ? ` el ${formatDate(change.applied_at)}` : ""}.`
            : `Descartado${
                change.discarded_by ? ` por ${change.discarded_by_kind === "agent" ? "el asistente de " : ""}${change.discarded_by}` : ""
              }. No se cambió nada.`}
        </div>
      )}
  ...
  return (
    <div className="mx-3.5 mt-2.5 overflow-hidden rounded-[11px] border border-(--line)">
      ...
      <div className="grid gap-2 px-3 py-2.5 text-[13px] leading-snug">
        <div>
          <div className="text-[11.5px] font-semibold text-(--ink-soft)">Antes</div>
          <p className="mt-0.5 whitespace-pre-line break-words text-(--ink-soft)">{formatValue(item.field, item.before)}</p>
        </div>
        <div>
          <div className="text-[11.5px] font-semibold text-(--ink)">Después</div>
  ...
  return (
    <div className="mx-3.5 mt-2.5">
      <div
        ...
        role="img"
        aria-label={`Precio nuevo ${formatMoney(item.after)}, anterior ${formatMoney(item.before)}, ${costLabel.toLowerCase()} ${formatMoney(cost)}`}
      >
      ...
      <p className="mt-1.5 text-[12.5px] tabular-nums text-(--ink-soft)">
        {costLabel} {formatMoney(cost)} · <b className="font-semibold text-(--ink)">{formatMoney(headroom)} de holgura</b> · {deltaPts >= 0 ? "+" : ""}
        {deltaPts.toFixed(1)} pts de margen
      </p>
    </div>
  );
}
...
export function PriceBand({ current, floor, ceiling }: { current: number; floor: number; ceiling: number }) {
  ...
  return (
    <div className="relative mt-1 h-[54px]" role="img" aria-label={`${formatMoney(current)} hoy, piso ${formatMoney(floor)}, techo ${formatMoney(ceiling)}`}>
