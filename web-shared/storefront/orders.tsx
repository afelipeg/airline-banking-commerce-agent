...
export const ORDER_NOUNS: OrderNouns = {
  one: "reserva",
  title: "Reservas",
  cardTitle: "Próximos viajes",
  noneOpen: "Sin viajes próximos",
  openVerb: "Sale el",
  closedWhen: (order, date) => (order.status === "delivered" ? `Volada el ${date}` : `Comprada el ${formatDayMonth(order.placed_at)}`),
  filters: [
    { id: "open", label: "Próximas", match: isOpen },
    { id: "delayed", label: "Retrasadas", match: (order) => order.status === "delayed" },
    { id: "closed", label: "Pasadas", match: (order) => !isOpen(order) },
  ],
  handoff(order) {
    const ref = `la reserva ${order.order_id}`;
    if (order.status === "delayed") return { label: "Preguntar por qué", prompt: `¿Por qué está retrasada ${ref} y cuándo sale?` };
    if (order.status === "delivered") return { label: "Preguntar", prompt: `¿Puedo pedir algún reembolso o ajuste de ${ref}?` };
    if (isOpen(order)) return { label: "Preguntar", prompt: `¿Cómo va ${ref}? ¿Qué me falta antes de viajar?` };
    return { label: "Preguntar", prompt: `¿Cuál es el estado de ${ref}?` };
  },
};
...
function orderTitle(order: Order): string {
  ...
  return rest.length ? `${first.title} + ${rest.length} más` : first.title;
}

function When({ order, nouns }: { order: Order; nouns: OrderNouns }) {
  const estimate = estimateOf(order);
  if (!estimate) return <span>Comprada el {formatDayMonth(order.placed_at)}</span>;
  ...
  return (
    <span className={order.status === "delayed" ? "font-semibold text-(--warn)" : ""} title={estimate.note ?? undefined}>
      {order.status === "delayed" ? "Prevista para el" : nouns.openVerb} {estimate.date}
    </span>
  );
}
...
export function ArrivingPanel({
  ...
  if (!orders) return failed ? <Notice>No pudimos cargar tus {nouns.title.toLowerCase()}.</Notice> : <Skeleton className="h-[188px]" />;
  ...
  return (
    <Panel
      ...
      action={onSeeAll ? <MoreLink label={`Todas las ${nouns.title.toLowerCase()}`} onClick={onSeeAll} /> : null}
    >
      <ul>
        {shown.map((order) => (
...
export function OrdersView({
  ...
  return (
    <StorePage>
      ...
      {orders === null ? (
        failed ? (
          <Notice>No pudimos cargar tus {title}. El asistente igual puede consultarlas.</Notice>
        ) : (
        ...
        <Notice>No hay {title} aquí.</Notice>
      )}
    </StorePage>
  );
}
