  ...
  return (
    <>
      <div className="flex items-center gap-2 border-b border-(--line) px-[18px] py-3.5">
        ...
        <IconButton icon="x" label={`Cerrar ${title.toLowerCase()}`} onClick={closePanel} className="ml-auto xl:hidden" />
      </div>
  ...
  return (
    <div className="flex items-center rounded-full border border-(--line-strong) bg-(--card)">
      <button
        ...
        aria-label={unit ? `Menos ${unit}s para ${itemTitle}` : `Disminuir la cantidad de ${itemTitle}`}
      ...
      <button
        ...
        disabled={busy}
        onClick={() => onChange(quantity + 1)}
        aria-label={unit ? `Más ${unit}s para ${itemTitle}` : `Aumentar la cantidad de ${itemTitle}`}
        className="px-2.5 py-0.5 text-sm text-(--ink-soft) hover:text-(--ink) disabled:opacity-40"
      >
        +
      </button>
    ...
    return (
      <button
        ...
        onClick={() => {
          ...
          else ask("Muéstrame otra vez el resumen de la compra.");
        }}
