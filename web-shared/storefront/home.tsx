...
const CATEGORY_LABELS: Record<string, string> = { preference: "Preferencia", constraint: "Regla", context: "Sobre ti" };
    ...
    <li className="border-t border-(--line) py-3 first:border-t-0">
      {draft === null ? (
        ...
        <div>
          ...
          />
          {failed ? <p className="mt-1 text-[12px] text-(--danger)">No se pudo guardar. Limítalo a una preferencia o una regla fija.</p> : null}
      ...
      <div className="mt-1.5 flex items-center gap-2 text-[11.5px] text-(--ink-soft)">
        {isNew ? <Pill tone="accent">Nuevo en esta sesión</Pill> : <Pill>{CATEGORY_LABELS[fact.category] ?? fact.category}</Pill>}
        {fact.updated_at ? <span>Guardado el {formatDayMonth(fact.updated_at)}</span> : null}
      </div>
    </li>
  );
}
    ...
    <Sheet title={name} detail={detail} onClose={onClose}>
      {others.length && onSwitchProfile ? (
        <div className="flex flex-wrap items-center gap-2 text-[13px] text-(--ink-2)">
          <span>Sesión iniciada como {name}.</span>
          {others.map((profile) => (
      ...
      <section>
        <h3 className="flex items-baseline gap-2 text-[15px] font-semibold text-(--ink)">
          Lo que {assistantName} sabe de ti
          <span className="ml-auto text-[12px] font-normal tabular-nums text-(--ink-soft)">{facts.length} guardados</span>
        </h3>
        <p className="mt-1 text-[13px] leading-snug text-(--ink-soft)">
          {assistantName} usa estos datos cuando te recomienda algo. Edita u olvida cualquier línea; lo que olvides se borra.
        </p>
        {facts.length ? (
          <ul className="mt-2">
            {facts.map((fact) => (
          ...
          <p className="mt-4 text-[13.5px] text-(--ink-2)">Nada guardado todavía.</p>
        )}
      </section>
      <p className="mt-auto border-t border-(--line) pt-3 text-[12px] leading-relaxed text-(--ink-soft)">
        Solo se guardan preferencias y reglas fijas. Nunca se guardan datos de tarjetas, cuentas, teléfonos ni correos.
      </p>
    </Sheet>
  );
}
